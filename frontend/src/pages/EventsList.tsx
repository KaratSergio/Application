        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4 mb-3 sm:mb-4 md:mb-6">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="relative w-full sm:w-auto sm:min-w-62.5 md:min-w-70">
              <MagnifyingGlassIcon className="absolute left-2 sm:left-3 top-1/2 transform -translate-y-1/2 w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5 text-gray-400" />
              <input
                type="text"
                placeholder="Search events..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full pl-8 sm:pl-9 md:pl-10 pr-7 py-1.5 sm:py-2 text-xs sm:text-sm md:text-base border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent bg-white"
              />
              {searchTerm && (
                <button
            ...
            <button
              ...
              className={`px-3 bg-white border border-gray-300 rounded-lg sm:px-3 py-1.5 sm:py-0 flex items-center justify-center space-x-1.5 transition-colors text-xs sm:text-sm cursor-pointer ${showFilters || activeFiltersCount > 0 ? ' text-green-700 ' : ' text-gray-400 '}`}
            >
              <FunnelIcon className="mr-0 w-3.5 h-3.5 sm:w-6 sm:h-6" />
              <span className="hidden xs:inline">Filters</span>
              {activeFiltersCount > 0 && (
                <span className="ml-1 bg-white text-green-600 rounded-full w-4 h-4 flex items-center justify-center text-[8px] sm:text-[10px] font-bold">
                  {activeFiltersCount}
                </span>
              )}
            </button>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-2 py-2.5 text-xs border bg-gray-50 border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                  />
                </div>
                <div>
        ...
        {filteredEvents.length > 0 ? (
          <div className="grid grid-cols-1 xs:grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 md:gap-4 lg:gap-6">
            {filteredEvents.map((event: Event) => {
              ...
              return (
                <div key={event.id} className="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all duration-300 flex flex-col h-full min-h-50 sm:min-h-60 md:min-h-70">
                  <Link to={`/events/${event.id}`} className="block flex-1">
                    <div className="p-3 sm:p-4 md:p-5 h-full flex flex-col">
                      <h2 className="text-sm sm:text-base md:text-lg font-semibold text-gray-900 transition-colors mb-1 line-clamp-2 h-10 sm:h-12">
                        {event.title}
                      </h2>
                  ...
                  <div className="px-3 pb-3 sm:px-4 sm:pb-4 md:px-5 md:pb-5 pt-0 mt-auto">
                    {event.organizerId === user?.id ? (
                      <DisabledButton fullWidth>You are the organizer</DisabledButton>
                    ) : event.isFull ? (
                      <FullButton fullWidth>Event Full</FullButton>
                    ) : event.userJoined ? (
                      isEventPassed(event.dateTime) ? (
                        <DisabledButton fullWidth>Event Ended</DisabledButton>
                      ) : (
                        <LeaveButton fullWidth onClick={(e) => { e.preventDefault(); leaveEvent(event.id); }}>
                          Leave Event
                        </LeaveButton>
                      )
                    ) : (
                      isEventPassed(event.dateTime) ? (
                        <EndedButton fullWidth>Event Ended</EndedButton>
                      ) : (
                        <JoinButton fullWidth onClick={(e) => handleJoinClick(e, event.id)}>
                          Join Event
                        </JoinButton>
                      )
                    )}
